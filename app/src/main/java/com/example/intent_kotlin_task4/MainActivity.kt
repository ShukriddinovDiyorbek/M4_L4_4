...
class MainActivity : AppCompatActivity() {
    ...
    private fun openSecondActivity(user: User) {
        ...
        getMemberFromSecondActivity.launch(intent)
    }

    private var getMemberFromSecondActivity = registerForActivityResult(
        ActivityResultContracts.StartActivityForResult()
    ) { res ->
        if (res.resultCode == Activity.RESULT_OK) {
            val data = res.data
            textView.text = data!!.getSerializableExtra("member").toString()
        }
    }
}